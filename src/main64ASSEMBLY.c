#include <stdio.h>
#include <stdlib.h>
#include <time.h>

#include "config.h"
#include "matrix.h"
#include "index.h"
#include "query64.h"
#include "compare64.h"
#include "distance.h"
#include "quantization.h"

// ---------------------------------------------
// Utility: millisecondi
// ---------------------------------------------
static double ms(clock_t start, clock_t end) {
    return 1000.0 * (double)(end - start) / CLOCKS_PER_SEC;
}

int main(int argc, char **argv)
{
    printf("argc = %d\n", argc);

#ifdef USE_AVX_ASM
    printf("[INFO] BUILD MODE : 64-bit DOUBLE\n");
    printf("[INFO] DISTANCE   : AVX2 ASM (external procedure)\n");
#elif defined(USE_AVX)
    printf("[INFO] DISTANCE   : AVX (intrinsics)\n");
#else
    printf("[INFO] DISTANCE   : SCALAR C\n");
#endif

    // -----------------------------------------------------
    // PARSING ARGOMENTI
    // -----------------------------------------------------
    Config cfg = {0};

    if (parse_args(argc, argv, &cfg) != 0) {
        printf("Uso:\n");
        printf("  %s -d dataset.ds2 -q query.ds2 -h <pivot> -k <vicini> -x <quant>\n",
               argv[0]);
        return 1;
    }

    printf("\n=============================\n");
    printf("   PARAMETRI DI INPUT (ASM)\n");
    printf("=============================\n");
    printf("Dataset : %s\n", cfg.ds_path);
    printf("Query   : %s\n", cfg.q_path);
    printf("h pivot : %d\n", cfg.h);
    printf("k       : %d\n", cfg.k);
    printf("x quant : %d\n\n", cfg.x);

    // -----------------------------------------------------
    // CARICAMENTO DATASET / QUERY (DOUBLE)
    // -----------------------------------------------------
    MatrixF64 ds = {0};
    MatrixF64 qs = {0};

    if (load_matrix_f64(cfg.ds_path, &ds) != 0) {
        printf("ERRORE: impossibile leggere dataset '%s'\n", cfg.ds_path);
        return 1;
    }

    if (load_matrix_f64(cfg.q_path, &qs) != 0) {
        printf("ERRORE: impossibile leggere query '%s'\n", cfg.q_path);
        free_matrix_f64(&ds);
        return 1;
    }

    if (ds.d != qs.d) {
        printf("ERRORE: dimensioni incompatibili (D dataset=%u, D query=%u)\n",
               ds.d, qs.d);
        free_matrix_f64(&ds);
        free_matrix_f64(&qs);
        return 1;
    }

    printf("Dataset caricato: %u x %u\n", ds.n, ds.d);
    printf("Query caricate : %u x %u\n\n", qs.n, qs.d);

    // -----------------------------------------------------
    // COSTRUZIONE INDICE
    // -----------------------------------------------------
    printf("Costruzione indice (64-bit)...\n");

    clock_t t0 = clock();
    Index *idx = build_index_f64(&ds, cfg.h, cfg.x);
    clock_t t1 = clock();

    if (!idx) {
        printf("ERRORE: build_index_f64 fallita.\n");
        free_matrix_f64(&ds);
        free_matrix_f64(&qs);
        return 1;
    }

    printf("Indice costruito.\n");
    printf("Tempo build_index(): %.2f ms\n\n", ms(t0, t1));

    // -----------------------------------------------------
    // KNN QUERY
    // -----------------------------------------------------
    int k = cfg.k;

    Neighbor64 *results = malloc(qs.n * k * sizeof(Neighbor64));
    if (!results) {
        printf("ERRORE: allocazione risultati fallita.\n");
        free_index(idx);
        free_matrix_f64(&ds);
        free_matrix_f64(&qs);
        return 1;
    }

    printf("Esecuzione K-NN (AVX2 ASM) su %u query...\n", qs.n);

    clock_t t2 = clock();
    knn_query_all_f64(&ds, idx, &qs, k, cfg.x, results);
    clock_t t3 = clock();

    printf("K-NN completato.\n");
    printf("Tempo knn_query_all(): %.2f ms\n\n", ms(t2, t3));

    // -----------------------------------------------------
    // CARICAMENTO RISULTATI UFFICIALI 64-BIT
    // -----------------------------------------------------
    MatrixI32 ref_ids = {0};
    MatrixF64 ref_dst = {0};

    if (load_matrix_i32("data/results_ids_2000x8_k8_x64_64.ds2", &ref_ids) != 0) {
        printf("ERRORE: lettura results_ids fallita.\n");
        goto cleanup;
    }

    if (load_matrix_f64("data/results_dst_2000x8_k8_x64_64.ds2", &ref_dst) != 0) {
        printf("ERRORE: lettura results_dst fallita.\n");
        goto cleanup;
    }

    // -----------------------------------------------------
    // CONFRONTO RISULTATI
    // -----------------------------------------------------
    printf("Confronto con i risultati ufficiali (ASM)...\n\n");

    compare_results_f64(&qs, results, &ref_ids, &ref_dst, k);

    // -----------------------------------------------------
    // BENCHMARK
    // -----------------------------------------------------
    printf("=====================================\n");
    printf("      BENCHMARK 64-bit ASM AVX2       \n");
    printf("=====================================\n");
    printf("build_index()     : %.2f ms\n", ms(t0, t1));
    printf("knn_query_all()   : %.2f ms\n", ms(t2, t3));
    printf("Totale runtime    : %.2f ms\n", ms(t0, t3));
    printf("=====================================\n\n");

cleanup:
    free_matrix_i32(&ref_ids);
    free_matrix_f64(&ref_dst);
    free(results);
    free_index(idx);
    free_matrix_f64(&ds);
    free_matrix_f64(&qs);

    printf("Esecuzione ASM completata correttamente.\n");
    return 0;
}
